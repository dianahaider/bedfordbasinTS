...
# Extract the sample metadata from Bedford
sample_metadata <- as.data.frame(sample_data(Bedford))
View(sample_metadata)
# Add a new column, for example, "RDA", indicating rows with or without NAs
sample_metadata$RDA <- ifelse(complete.cases(sample_metadata), "Yes", "No")
# Reassign the modified metadata back to Bedford
sample_data(Bedford) <- sample_metadata
...
#Sorting the RDA table
BB_RDA_CLR_t <- t(BB_RDA_CLR_abund)
BB_RDA_CLR.t.sort <- as.data.frame(BB_RDA_CLR_t)
BB_RDA_CLR.t.sort <- cbind(BB_RDA_CLR.t.sort, X3$Month) #column that you want to color the dot
...
BB_RDA_CLR.t.sort.rda <- rda(
  BB_RDA_CLR.t.sort ~Chla+Nitrate+PO4+Sal+Si+Temp+DO+DL, #######here add your significant parameters
  # ,
  data = X3.wf.data) #+DL
