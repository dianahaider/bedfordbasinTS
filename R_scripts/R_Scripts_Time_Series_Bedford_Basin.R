...
######## Import data
#subselection in data
otu_mat <-"data/subyears_tables.csv"
tax_mat <- "data/allyears_tax.csv"
samples_df <- "data/env_data_deep.csv"

#subselection of 2014-2017
otu_mat <-"data/y1417/subyears_tables.csv"
tax_mat <- "data/y1417/allyears_tax.csv"
samples_df <- "data/y1417/env_data_deep2.csv"

#subselection from Eric's paper ran with Deblur
otu_mat <-"data/ASV_Table_Bedford_Basin.csv"
tax_mat <- "data/Taxonomy_species_Silva138.1_Bedford_Basin.csv"
samples_df <- "data/METADATA_Bedford_Basin.csv"
...
######## Remove taxa
Bedford_no_mito = Bedford
Bedford_no_mito = subset_taxa(Bedford, !Domain=="NA" )
Bedford_no_mito = subset_taxa(Bedford_no_mito, !Domain=="d__Eukaryota" )
Bedford_no_mito = subset_taxa(Bedford_no_mito, !Phylum=="NA" )
Bedford_no_mito = subset_taxa(Bedford_no_mito, !Order=="o__Chloroplast" )
Bedford_no_mito = subset_taxa(Bedford_no_mito, !Family=="f__Mitochondria" )
...
sample_data(Bedford_no_mito)$month_name = factor(sample_data(Bedford_no_mito)$month_name, levels=c("Jan", "Feb", "Mar", "Apr","May", "Jun","Jul", "Aug", "Sep","Oct", "Nov", "Dec"))
sample_data(Bedford_no_mito)$Year = factor(sample_data(Bedford_no_mito)$Year, levels=c("2014", "2015", "2016", "2017"))
...
p3+ stat_ellipse(type = "norm", linetype = 2, level =0.9) +
  stat_ellipse(type = "t", level=0.9)

#> Too few points to calculate an ellipse
#> Warning: Removed 1 rows containing missing values (geom_pa
#p3$layers <- p3$layers[-1]

p3+ggtitle("PCA Bedford 16S rRNA ASV Deblur 2014-2017")+
  ...
  geom_point(size=3) + scale_colour_manual(values=my.cols) + stat_ellipse(type = "norm", linetype = 2, level =0.9) +
  stat_ellipse(type = "t", level=0.9)
...
View(BB_RDA_CLR.t.sort)


BB_RDA_CLR.t.sort.rda <- rda(
  BB_RDA_CLR.t.sort ~Chla+NO3+PO4+Sal+Si+Temp+DO, #######here add your significant parameters
  # ,
  data = X3.wf.data) #+DL
